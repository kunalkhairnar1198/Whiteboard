  ...
  ({ className, side = 'left', mobileOpen = false, onToggle, children, ...props }, ref) => (
    ...
    >
      ...
      {onToggle && (
        ...
        >
          {side === 'left' ? (
            <ChevronLeft className={cn("h-4 w-4 transition-transform duration-300", !mobileOpen && "rotate-180")} />
          ) : (
            <ChevronRight className={cn("h-4 w-4 transition-transform duration-300", !mobileOpen && "rotate-180")} />
          )}
        </Button>
      )}
    </aside>
