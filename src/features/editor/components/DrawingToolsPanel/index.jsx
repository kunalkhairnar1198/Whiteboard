...
const DrawingToolsPanel = ({
  ...
  return (
    <div className="space-y-6">
      {/* Tools */}
      <div>
        <h3 className="mb-3 font-semibold text-foreground text-sm">Tools</h3>
      ...
      {/* Shapes */}
      <div>
        <h3 className="mb-3 font-semibold text-foreground text-sm">Shapes</h3>
        <div className="gap-2 grid grid-cols-4">
          {shapes.map((shape) => (
            <button
              key={shape.type}
              onClick={() => handleAddShape(shape.type)}
              className="hover:bg-secondary p-3 border border-border hover:border-primary/50 rounded-lg transition-all"
              title={shape.label}
            >
        ...
        <div className="space-y-2 mt-3">
          <label className="block text-muted-foreground text-xs">Corner/Radius</label>
          <input
            type="range"
            min="0"
            max="200"
      ...
      {/* Text */}
      <div>
        <h3 className="mb-3 font-semibold text-foreground text-sm">Text</h3>
        <button
          onClick={() => handleAddShape('text')}
          className="flex justify-center items-center gap-2 bg-primary hover:bg-primary/80 p-3 rounded-lg w-full text-primary-foreground transition-all"
        >
          <Type className="w-5 h-5" />
          Add Text
        </button>
        <div className="mt-4">
          <label className="block mb-1 text-muted-foreground text-xs">Text Color:</label>
          <input
            type="color"
            value={textColor}
            onChange={(e) => setTextColor(e.target.value)}
            className="border border-border rounded-lg w-full h-8 cursor-pointer"
          />
        </div>
      </div>
      ...
      {/* Images */}
      <div>
        <h3 className="mb-3 font-semibold text-foreground text-sm">Images</h3>
        <label className="block hover:bg-secondary p-8 border-2 border-border hover:border-primary/50 border-dashed rounded-lg text-center transition-all cursor-pointer">
